/** Luxury architecture URLs — 6 frames per slug, one building each (validated Unsplash only). */
...
const MULTI_PHOTO_SETS: readonly (readonly string[])[] = [
  [
    "https://images.unsplash.com/photo-1600585154340-be6161a56a0c",
    "https://images.unsplash.com/photo-1600585154526-990dced4db0d",
    "https://images.unsplash.com/photo-1600585152915-d208bec867a1",
...
function cropVariants(base: string): string[] {
...
/** Six Unsplash URLs for one slug — same luxury building throughout. */
export function buildingSetForSlug(slug: string): string[] {
...
export function buildingImageUrlForSlug(slug: string, index: number): string {
  const set = buildingSetForSlug(slug);
  return set[Math.max(0, Math.min(5, index)) % set.length]!;
}
