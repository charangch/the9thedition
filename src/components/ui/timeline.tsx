...
const timelineItemVariants = cva("relative flex gap-3 pb-2", {
  variants: {
    orientation: {
      vertical: "flex-row",
      horizontal: "relative min-w-64 shrink-0 flex-col",
    },
  },
  defaultVariants: {
    orientation: "vertical",
  ...
  "flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 bg-background text-xs font-medium",
  {
    variants: {
      status: {
        ...
        active: "animate-pulse border-primary bg-background text-primary",
...
export function Timeline({
  ...
  if (orientation === "horizontal") {
    return (
      <ScrollArea orientation="horizontal" className={cn("w-full max-w-full", className)}>
        {timelineContent}
      </ScrollArea>
    );
  }
